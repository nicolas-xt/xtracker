...
interface PayoffByStrategyChartProps {
  stats: DetailedStats;
}

export const PayoffByStrategyChart: React.FC<PayoffByStrategyChartProps> = ({ stats }) => {
  const data = stats.payoffByStrategy.map(item => ({
  ...
  return (
    <div className="bg-gradient-to-br from-[#242424]/60 via-[#2A2A2A]/40 to-[#242424]/60 backdrop-blur-[20px] border border-gray-500/20 rounded-[20px] p-6 shadow-md relative overflow-hidden">
      ...
      <div className="relative">
        <h3 className="text-lg font-semibold text-white mb-4">Payoff Médio por Estratégia</h3>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.1)" />
            <XAxis dataKey="strategy" stroke="rgba(255,255,255,0.7)" />
            ...
            />
            <Bar dataKey="avgPayoff" name="Payoff Médio" fill="#00D0FF" />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
